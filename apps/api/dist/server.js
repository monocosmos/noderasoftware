...
function canCreateWorkOrderForDepartment(auth, type, targetDepartmentId, requestMode = "incoming") {
...
function isFourDigitDepartmentCode(code) {
    return /^\d{4}$/.test(code);
}
async function nextCustomDepartmentCode(hotelId) {
...
async function accountDeleteRequestRecipient(auth) {
...
function positiveQueryInteger(value, fallback) {
    const raw = Array.isArray(value) ? value[0] : value;
...
function paginatedItemsResponse(items, page, pageSize, total) {
    return {
        items,
...
function queryDateBoundary(value, endOfDay = false) {
...
function workOrderListWhere(auth, query) {
    const filters = [workOrderVisibilityWhere(auth)];
...
async function canDelayWorkOrder(auth, departmentId, departmentDbId) {
    if (auth.departmentId !== departmentId)
        return false;
    if (canManageWorkOrderStatus(auth, departmentId))
        return true;
...
function canDeleteOriginDepartmentWorkOrder(auth, workOrder) {
    const targetDepartmentId = clientDepartmentIdFromCode(workOrder.department.code);
    const originDepartmentId = workOrder.createdBy?.department?.code
        ? clientDepartmentIdFromCode(workOrder.createdBy.department.code)
        : "";
    return (Boolean(originDepartmentId) &&
...
async function outsideShiftSoundPreferenceUserIdSet(userIds) {
    if (!userIds.length)
        return new Set();
    const users = await prisma.user.findMany({
...
function ensureTechnicalMeterScope(req, res) {
...
function serializeMeterTrackingTable(table, req) {
...
async function findMeterTrackingTable(hotelId) {
...
app.get("/meter-tracking", authenticate, requireModuleAccess("meterTracking"), asyncHandler(async (req, res) => {
    if (!ensureTechnicalMeterScope(req, res))
        return;
...
app.get("/work-orders", authenticate, requirePermission("work-orders:read"), requireModuleAccess("jobs"), async (req, res) => {
    ...
    res.json(paginatedItemsResponse(workOrders.map((workOrder) => serializeWorkOrder(workOrder)), listPage.page, listPage.pageSize, total));
});
...
app.patch("/work-orders/:code", authenticate, requirePermission("work-orders:update"), requireModuleAccess("jobs"), async (req, res) => {
    ...
    if (payload.locationDetail !== undefined)
        data.locationDetail = payload.locationDetail;
...
app.get("/operation-documents", authenticate, requireModuleAccess("operationDocuments"), asyncHandler(async (req, res) => {
    ...
    const where = { hotelId: req.auth.hotelId, deletedAt: null };
...
app.get("/audit-logs", authenticate, requirePermission("audit:read"), requireModuleAccess("reports"), requireFeatureAccess("featureAuditLogs"), async (req, res) => {
    const listPage = paginationFromQuery(req.query);
    const where = req.auth.roleId === "generalManager"
