...
import type { Attachment, DepartmentCode, Notification as DbNotification, PushDevice, User, UserProfileTemplate } from "@prisma/client";
...
let maintenanceStatusCache: { status: MaintenanceStatus; expiresAt: number } | null = null;
...
function cacheAuthenticatedRequest(token: string, auth: AuthContext, user: AuthedUser, sessionExpiresAt: Date) {
  authCache.set(token, {
    auth,
    user,
...
function cachedAuthenticatedRequest(token: string) {
...
function invalidateAuthCache(predicate?: (entry: AuthCacheEntry) => boolean) {
  if (!predicate) {
    authCache.clear();
    return;
  }
...
function invalidateAuthCacheForUser(userId: string) {
  invalidateAuthCache((entry) => entry.auth.userId === userId);
}

function invalidateAuthCacheForSession(sessionId: string) {
...
function hasFeatureAccess(req: express.Request, featureId: string) {
  try {
    ...
    if (featureId === "featureHotelFloorPlanning" || featureId === "featureMeterTrackingEdit" || featureId === "accountDeleteRequest") return access[featureId] === true;
    ...
    if (featureId === "featureHotelFloorPlanning" || featureId === "featureMeterTrackingEdit" || featureId === "accountDeleteRequest") return false;
    if (req.auth?.roleId === "generalManager") return true;
    if (featureId === "managementRequests" || featureId === "reports" || featureId === "featureDailyReport") return true;
    return true;
  }
...
function canManageHotelFloorPlan(req: express.Request) {
...
function serializeHotelFloorArea(area: SerializableHotelFloor["areas"][number]) {
...
function hotelFloorAreaVisibleForDepartment(area: ReturnType<typeof serializeHotelFloorArea>, departmentId: string) {
  if (!area.visibleToDepartments) return area.visibleDepartmentIds.includes(departmentId);
  if (!area.visibleDepartmentIds.length) return true;
...
function generatedWorkOrderCode(prefix: string) {
...
function parseModuleAccessJson(value: string | null | undefined) {
  try {
    const parsed = JSON.parse(value || "{}") as Record<string, unknown>;
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) return {};
    return Object.fromEntries(Object.entries(parsed).filter(([, enabled]) => typeof enabled === "boolean")) as Record<string, boolean>;
  } catch {
    return {};
  }
}
...
function serializeUserProfileTemplate(
...
const userProfileTemplateInclude = { role: true, department: true, createdBy: { select: { id: true, fullName: true } } } as const;
...
type SyncStateSnapshot = Awaited<ReturnType<typeof buildSyncState>>;

type SyncStateCacheEntry = {
  expiresAt: number;
  snapshot?: SyncStateSnapshot;
  pending?: Promise<SyncStateSnapshot>;
};

const syncStateCache = new Map<string, SyncStateCacheEntry>();

function invalidateSyncStateCache() {
  syncStateCache.clear();
}

function syncStateCacheKey(req: express.Request) {
  const auth = req.auth!;
  const user = req.user!;
  return [
    auth.hotelId,
...
async function getCachedSyncState(req: express.Request) {
  if (syncStateCacheTtlMs <= 0) return buildSyncState(req);
...
function shiftWindowContains(window: { start: Date; end: Date }, now: Date) {
...
async function cancelShiftStatusPush(userId: string) {
  await sendAndroidDataPushToUser(userId, {
...
async function closeOpenShiftsOutsideScheduledWindow(userIds: string[], now: Date) {
  const uniqueUserIds = Array.from(new Set(userIds));
...
async function processShiftStartReminders() {
  ...
  if (!cells.length && !activeShiftUserIds.size) return;

  const validReminderKeys = new Set<string>();
  const currentWindowUserIds = new Set<string>();
...
const reminderRequestLastCheckedAt = new Map<string, number>();

async function processDueRemindersForRequest(userId: string) {
  const now = Date.now();
  const lastCheckedAt = reminderRequestLastCheckedAt.get(userId) ?? 0;
  if (now - lastCheckedAt < reminderRequestCheckIntervalMs) return;
...
app.patch("/profile-templates/:id", authenticate, requirePermission("users:write"), asyncHandler(async (req, res) => {
  ...
  const data: Prisma.UserProfileTemplateUpdateInput = {};
  if (payload.name) data.name = payload.name;
  if (payload.description !== undefined) data.description = payload.description ?? null;
  if (payload.shiftTrackingEnabled !== undefined) data.shiftTrackingEnabled = payload.shiftTrackingEnabled;
  if (payload.moduleAccess !== undefined) data.moduleAccessJson = JSON.stringify(payload.moduleAccess);
...
app.patch("/notifications/read-all", authenticate, async (req, res) => {
  ...
  if (result.count) invalidateSyncStateCache();
